...
bool my_update(const sensor_msgs::LaserScan& input_scan)
{
  ...
  filtered_scan.angle_min = 3.1415+0.0417;
  filtered_scan.angle_max = 3.1415-0.0417;
...
int main(int argc, char** argv){
  ros::init(argc, argv, "rplidar_bound_filter");
  ros::NodeHandle n1, n2;
  input_scan_sub = n1.subscribe("/scan", 10, input_scan_sub_callback);
  filtered_scan_pub = n2.advertise<sensor_msgs::LaserScan>("/rplidar_bound_scan", 50);
